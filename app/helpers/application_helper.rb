  ...
  def illustrated_markdown_to_html(id, type, markdown_text)
    ...
    html.css('img').each do |img|
      ...
      if illustration = Illustration.where(:illustratable_id => id, 
        ...
        img.set_attribute('data-popover-src', illustration.illustration.url(:popover))
        popover_image_file = Paperclip::Geometry.from_file(illustration.illustration.path(:popover))
        img.set_attribute('data-popover-width', popover_image_file.width.to_i.to_s)
        img.set_attribute('data-popover-height', popover_image_file.height.to_i.to_s)
      else
    ...
    allowed_attributes['img'] += ['id', 'width', 'height', 'data-popover-src', 'data-popover-width', 'data-popover-height']
    html = Sanitize.clean(html, :elements => allowed_elements, :attributes => allowed_attributes, 
      :protocols => allowed_protocols)
